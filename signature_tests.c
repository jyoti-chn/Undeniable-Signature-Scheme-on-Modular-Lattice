...
void test_Sign_and_Verify(int n)
	...
	KeyGen2(A, T, cplx_T, sch_comp,h,sd,v,L);
	...
	poly_matrix sig_2 = nu_coeffs;
	
	printf("Signing...\n");
	printf("\n");
	scalar* sig_1 = malloc( PARAM_N*PARAM_D*sizeof(scalar) ) ;

	scalar* sig_3 = malloc( PARAM_N*PARAM_D*sizeof(scalar) ) ;

	scalar* M = malloc(PARAM_N * PARAM_D * (PARAM_M - PARAM_D) * sizeof(scalar));
	for(int i = 0 ; i < n ; ++i)
		{
			
		Sign2(sig_2, r, A, T, cplx_T, sch_comp, m, MESSAGE_BYTES,h,v , sig_1 , sig_3 , M)  ;
		
			
		if(Verify2(sig_1 ,sig_2, sig_3, r, A ,h ,sd , m, MESSAGE_BYTES , v ,L,M ) == false)
			{
