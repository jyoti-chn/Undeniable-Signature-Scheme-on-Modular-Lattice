...
/*
	Converts the (uniform) bytes from r to a poly f of degree deg
		assumes that enough bytes are provided in r
*/
void uniform_poly_from_bytes(poly f, uint8_t *r_in, int deg)
	...
	scalar *r = (scalar *) r_in;
	int f_index = 0;
	
	for(int r_index = 0 ; r_index < NEEDED_XOF_BYTES ; ++r_index)
		{
		if(r[r_index] < past) // r[i] is small enough so that it can be mapped to [0, q-1]
			{
			f[f_index] = r[r_index] / scaling;
			f_index++;
			}
		if(f_index == deg + 1) // all coefficients are done
			{
			return;
			}
		}
	
	// This code should be executed with negligible probability
	//memset(&f[f_index], 0, deg + 1 - f_index);
	}

/*
	t <- H(m || r)
		t is in R^d in the CRT domain
		m is the message of length m_len to be signed
		r is a salt of length SALT_BYTES
*/

void hash_com(poly_matrix t, uint8_t* r  ){

  int len = SALT_BYTES ;
  
  for(int i=0; i< (PARAM_M-PARAM_D) ; i++ ){
      uint8_t XOFmr[NEEDED_XOF_BYTES];
	    shake256(XOFmr, NEEDED_XOF_BYTES, r, len);

      int k = 0 ;
      for( uint8_t* j = r ; j<r + len ; j++ ){
          *j = (*j)^ XOFmr[k] ;
      }
	    // t <- R_q^d(Hmr)
	    uniform_poly_from_bytes(t+i*(PARAM_N*PARAM_D), XOFmr, PARAM_N * PARAM_D - 1);
  }

}

void Hash_com_message(poly t, uint8_t *m, int m_len, uint8_t *r)
	{
	int mr_len = m_len + SALT_BYTES;
	uint8_t mr[mr_len];
	
	// mr <- m || r
	memcpy(mr, m, m_len);
	memcpy(&mr[m_len], r, SALT_BYTES);
	
	  for(int i=0; i< (PARAM_M-PARAM_D) ; i++ ){
      uint8_t XOFmr[NEEDED_XOF_BYTES];
	    shake256(XOFmr, NEEDED_XOF_BYTES, mr, mr_len);

      int k = 0 ;
      for( uint8_t* j = mr ; j<mr + mr_len ; j++ ){
          *j = (*j)^ XOFmr[k] ;
      }
	    // t <- R_q^d(Hmr)
	    uniform_poly_from_bytes(t+i*(PARAM_N*PARAM_D), XOFmr, PARAM_N * PARAM_D - 1);
...
void H(poly t, uint8_t *m, int m_len, uint8_t *r)
	...
	*/
	...
	/*
	printf("Hmr\n[");
	for(int i = 0 ; i < NEEDED_XOF_BYTES ; ++i)
		{
		printf("%02" PRIX8 ", ", Hmr[i]);
		}
	printf("]\n\n");
	*/
	
	// t <- R_q^d(Hmr)
	uniform_poly_from_bytes(t, XOFmr, PARAM_N * PARAM_D - 1);
	}

void H_2(poly t, uint8_t *m, int m_len)
	{
	int mr_len = m_len ;
	uint8_t mr[mr_len];
